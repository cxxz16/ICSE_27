...
static zval *viper_get_op2_zval(const zend_op *opline, zend_execute_data *execute_data)
{
    if (!opline || !execute_data) return NULL;
    switch (opline->op2_type) {
        case IS_CONST:   return RT_CONSTANT(opline, opline->op2);
        case IS_TMP_VAR:
        case IS_VAR:
        case IS_CV:      return EX_VAR(opline->op2.var);
        case IS_UNUSED:
        default:         return NULL;
    }
}
...
static void viper_emit_cmp(const zend_op *, zend_execute_data *, const char *, u64, long long);
static void viper_emit_switch(const zend_op *, zend_execute_data *, const char *, u64, long long);
...
static void (*viper_orig_zend_error_cb)(int, const char *, const uint32_t,
                                         const char *, va_list);
static void viper_zend_error_cb(int type, const char *error_filename,
                                ...
                                const char *current_filename, u64 current_lineno)
{
    ...
    switch (opline->opcode) {
        ...
        case ZEND_CASE:
            viper_emit_cmp(opline, execute_data, current_filename, current_lineno, dist_now);
            break;
        case ZEND_SWITCH_STRING:
        case ZEND_SWITCH_LONG:
            viper_emit_switch(opline, execute_data, current_filename, current_lineno, dist_now);
            break;
                          ...
                          long long distance)
{
    ...
    switch (opline->opcode) {
        ...
        case ZEND_JMPZNZ:   opname = "JMPZNZ";   jumped =  1;      break;
        default:            return;
    }
...
static void viper_emit_cmp(const zend_op *opline, zend_execute_data *execute_data,
                           const char *current_filename, u64 current_lineno,
                           long long distance)
{
    if (viper_blocker_disabled) return;
    ...
    zval *op1 = viper_get_op1_zval(opline, execute_data);
    zval *op2 = viper_get_op2_zval(opline, execute_data);


    if (op1 && (Z_TYPE_P(op1) == IS_ARRAY || Z_TYPE_P(op1) == IS_OBJECT)) return;

    const char *t1 = "unknown", *t2 = "unknown";
    char b1[512], b2[512];
    viper_zval_repr(b1, sizeof(b1), op1, &t1);
    viper_zval_repr(b2, sizeof(b2), op2, &t2);

    const char *opname = "?";
    switch (opline->opcode) {
        case ZEND_IS_EQUAL:         opname = "IS_EQUAL";         break;
        case ZEND_IS_NOT_EQUAL:     opname = "IS_NOT_EQUAL";     break;
        case ZEND_IS_IDENTICAL:     opname = "IS_IDENTICAL";     break;
        case ZEND_IS_NOT_IDENTICAL: opname = "IS_NOT_IDENTICAL"; break;
        case ZEND_CASE:             opname = "CASE";             break;
        default:                    return;
    }
...
static void viper_emit_switch(const zend_op *opline, zend_execute_data *execute_data,
                              const char *current_filename, u64 current_lineno,
                              long long distance)
{
    if (viper_blocker_disabled) return;
    ...
    zval *op1 = viper_get_op1_zval(opline, execute_data);
    const char *t1 = "unknown";
    char b1[512];
    viper_zval_repr(b1, sizeof(b1), op1, &t1);

    const char *opname = (opline->opcode == ZEND_SWITCH_LONG) ? "SWITCH_LONG"
                                                              : "SWITCH_STRING";
    char file_buf[768];
    viper_json_escape(file_buf, sizeof(file_buf), current_filename);
    ...
    fprintf(viper_blocker_fp,
        ...
        "\"operands\":{\"lhs\":{\"value\":\"%s\",\"type\":\"%s\"}},"
        "\"cases\":[",
        file_buf, current_lineno, opname, distance, b1, t1);


    zval *jt = viper_get_op2_zval(opline, execute_data);
    int first = 1;
    if (jt && Z_TYPE_P(jt) == IS_ARRAY) {
        zend_string *key; zend_ulong idx;
        HashTable *ht = Z_ARRVAL_P(jt);
        ZEND_HASH_FOREACH_KEY(ht, idx, key) {
            char kbuf[256], esc[512];
            if (key) {
                size_t l = ZSTR_LEN(key); if (l > 200) l = 200;
                memcpy(kbuf, ZSTR_VAL(key), l); kbuf[l] = '\0';
            } else {
                snprintf(kbuf, sizeof(kbuf), "%lld", (long long)idx);
            }
            viper_json_escape(esc, sizeof(esc), kbuf);
            fprintf(viper_blocker_fp, "%s\"%s\"", first ? "" : ",", esc);
            first = 0;
                          ...
                          long long distance, const char *opname, const char *mechanism)
{
    ...
    if (opline) {
        ...
        if (op1 && is_throw && Z_TYPE_P(op1) == IS_OBJECT) {
            ...
            zval rv;
            zval *m = zend_read_property(obj->ce, op1, "message",
                                 ...
                                 const char *format, va_list args)
{
    ...
    if (!viper_blocker_disabled && viper_blocker_fp) {
        ...
        vsnprintf(msg_raw, sizeof(msg_raw), format, args_copy);
        va_end(args_copy);

        char msg_json[2048];
        char file_json[768];
        viper_json_escape(msg_json,  sizeof(msg_json),  msg_raw);
    ...
    if (viper_orig_zend_error_cb) {
        viper_orig_zend_error_cb(type, error_filename, error_lineno,
                                  format, args);
    }
}
